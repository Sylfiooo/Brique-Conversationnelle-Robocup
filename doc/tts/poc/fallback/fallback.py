...
class CoquiTTSPlayer:
    def __init__(self):
        ...
        # self.tts = TTS("tts_models/en/ljspeech/fast_pitch") # => around 1Gb | less fast but more comprhensive, around 0.8 sec for 2 sentences
        # self.tts = TTS("tts_models/en/ljspeech/speedy-speech") # => around 1Gb | pretty fast and comprhensive, around 0.4 sec for 2 sentences
        self.tts = TTS("tts_models/en/ljspeech/glow-tts") # => around 400Mb | good comprehensive, around 0.3 sec for 2 sentences but long for the first

        # self.tts = TTS("tts_models/en/ljspeech/tacotron2-DDC") # => around 100Mb | very comprehensive, around 2 sec for 2 sentences but 4 sec for the first
        # self.tts = TTS("tts_models/en/ljspeech/vits") # => around 100Mb | very comprehensive, around 2 sec for 2 sentences but 6 sec for the first
...
class FallBackTTSManager:
    def __init__(self, pico_tts_player, coqui_tts_player):
        self.input_history = []
        self.pico_tts_player = pico_tts_player
        self.coqui_tts_player = coqui_tts_player
    ...
    def loop(self):
        ...
        while True:
            ...
            if self.check_wifi_connection():
                self.coqui_tts_player.generate_and_play_tts(text_to_speak)
            else:
...
if __name__ == "__main__":
    ...
    coqui_tts_player = CoquiTTSPlayer()
    fallback_tts_manager = FallBackTTSManager(pico_tts_player, coqui_tts_player)
